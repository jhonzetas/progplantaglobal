...
// Metas diarias. Cambiar aquí si la planta las ajusta; alimentan el marcador
// del encabezado (meta · turno anterior · % de cumplimiento).
...
function fechaCorta(iso: string): string {
  ...
  if (!m) return iso;
  const [, y, mes, dia] = m;
  return `${parseInt(dia, 10)}/${parseInt(mes, 10)}/${y.slice(2)}`;
}
...
export default function Kiosko() {
  ...
  const [produccionAnterior, setProduccionAnterior] = useState<
    { unidades: number; fecha: string; montajes: number | null } | null
  >(null);
  ...
  const poll = useCallback(async () => {
    try {
      ...
      const rProd = await fetch("/api/produccion", { cache: "no-store" });
      if (rProd.ok) {
        setProduccionAnterior(await rProd.json());
      }
  ...
  // Wrapper que registra en el historial de Deshacer para poder revertir
  // un "quitar marca" accidental. La función limpiarEstado de abajo no lo
  // hace por sí sola porque también la usa el flujo de deshacer.
  async function quitarMarca(opId: string) {
    const valorAnterior = estadoRef.current[opId] as Estado | undefined;
    if (!valorAnterior) return;
    setHistorial((prev) => [...prev, { id: opId, valorAnterior }].slice(-5));
    await limpiarEstado(opId);
  }
  ...
  return (
    <div className="h-screen w-screen overflow-hidden select-none bg-panel text-ink font-display flex flex-col">
      ...
      {(() => {
        const prod = produccionAnterior;
        const pctU = prod ? (prod.unidades / OBJETIVO_PRODUCCION_DIARIA) * 100 : null;
                ...
                >
                  ...
                  <td className="p-1 border-r-2 border-r-amber">
                    <div className="flex flex-col gap-1">
                      ...
                      {actual && (
                        <button
                          onClick={() => quitarMarca(fila.ID)}
                          aria-label="Quitar marca de esta fila"
                          className="w-full py-1 rounded font-display font-extrabold text-[10px] max-md:text-[8px] leading-none tracking-wide border border-signal-red/60 bg-signal-red/15 text-signal-red hover:bg-signal-red/25"
                        >
                          ✕ QUITAR
                        </button>
                      )}
